...
import uguess.qucai.com.merchant.framework.protocol.FileImageUpload;
import uguess.qucai.com.merchant.framework.ui.helper.Alert;

/**
 * Created by NO1 on 2015/3/31.
 */
public class CrashHandler implements Thread.UncaughtExceptionHandler {
    ...
    private static final String upload_url ="/common/upload.html?fileUsage=3";
...
//                Toast.makeText(mContext, "程序出错啦:" + msg, Toast.LENGTH_LONG).show();
    ...
    private String saveCrashInfoToFile(Throwable ex) {
        ...
        try {
            ...
            String fileName = "crash_" + timestamp +"_"+ Cache.getInstance().getUser().getCellNum()+ CRASH_REPORTER_EXTENSION;
            // 保存文件
            File dir = new File(Const.WorkDir+"log/");
            if(!dir.exists()){
                dir.mkdir();
            }
            ...
            public void run() {
                ...
                if (crFiles != null && crFiles.length > 0) {
                    ...
                    for (String fileName : sortedFiles) {
                        File cr = new File(Const.WorkDir+"log/", fileName);
                        postReport(cr);
                        cr.delete();// 删除已发送的报告
                    }
                }
    ...
    private String[] getCrashReportFiles(Context ctx) {
        File filesDir = new File(Const.WorkDir+"log/");
